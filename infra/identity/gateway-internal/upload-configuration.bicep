...
resource uploadScript 'Microsoft.Resources/deploymentScripts@2023-08-01' = {
  name: 'upload-internal-gateway-routes'
  location: location
  kind: 'AzureCLI'
  properties: {
    azCliVersion: '2.52.0'
    ...
    scriptContent: '''
      ...
      az storage file delete \
        ...
        --path kong-internal.yml 2>/dev/null || true

      echo "$AZURE_ROUTES_CONTENT" > kong-internal.yml
      az storage file upload \
        --account-name "$STORAGE_ACCOUNT_NAME" \
        --account-key "$STORAGE_ACCOUNT_KEY" \
        --share-name "$FILE_SHARE_NAME" \
        --source kong-internal.yml \
        --path kong-internal.yml
    '''
  }
}
