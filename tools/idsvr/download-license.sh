...
function getLicenseToolDownloadFileName() {
  ...
  case "$(uname -s)" in
    ...
    echo "curity-license-cli-${CLI_VERSION}-${PLATFORM}-${ARCH}.zip"
}

#
# Exit if there is already a license file, like one copied in manually
#
...
#
if [ ! -f ./license.json ]; then
  echo 'This script gets a trial license file for the Curity Identity Server.'
